...
public class HttpsUtil {
    ...
    private static final String[] CERTIFICATES = new String[]{"123.cer"};
    ...
    /**
     * HttpUrlConnection支持双向Https验证（其中getSSLSocketFactory方法的后两个参数需要传入合适的值）
     * @param context
     */
    public static void initDoubleHttpsUrlConnection(Context context) {
        InputStream[] certificates = getCertificates(context, CERTIFICATES);
        SSLSocketFactory sslSocketFactory = getSSLSocketFactory(certificates, null, null);//其中getSSLSocketFactory方法的后两个参数需要传入合适的值
        HttpsURLConnection.setDefaultSSLSocketFactory(sslSocketFactory);
        if (certificates == null) {
            HttpsURLConnection.setDefaultHostnameVerifier(getUnSafeHostnameVerifier());
        }
